...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        x = F.relu(F.batch.norm(self.conv1(x.float())))
        x = F.relu(F.batch.norm(self.conv2(x)))
        x = F.max_pool2d(x, kernel_size=2, stride=2)

        x = F.relu(F.batch.norm(self.conv3(x)))
        x = F.relu(F.batch.norm(self.conv4(x)))
        x = F.relu(F.batch.norm(self.conv5(x)))
        x = F.max_pool2d(x, kernel_size=2, stride=2)
